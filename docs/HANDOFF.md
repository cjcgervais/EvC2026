...
## ▶ COLD START — read this box first (post-Session 25/26, 2026-07-02)
...
1. **Cursor circle → 5/8 (S26, IN TREE, needs his Play).** Chad: *"the cursor range now allows it to go all the way to the edges and corners; my cursor circle should be centered and take up only about 5/8 of the total area of the screen."* → `Controls.aimLeadScreenFrac 0.95→0.625` (the knob IS the cursor-circle size, same semantics as his prior 7/8 and 1/2 tunes; CS-8 model untouched). ⚠️ Interaction noted: at 0.625 the pinned-cursor max lead ≈ 23.6°, so a held-edge cursor's sustained roll command ≈ 0.9 (was saturated 1.0) — pitch still saturates (loops fine). If max-rate turns from a pinned cursor feel weaker, compensate with `aimRollGain`, don't re-grow the circle.
